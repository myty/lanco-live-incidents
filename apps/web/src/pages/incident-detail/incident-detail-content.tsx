...
export const IncidentDetailContent: React.FC<IncidentDetailContentProps> = ({ incident }) => {
  ...
  const currentPosition =
    position != null ? { lat: position.coords.latitude, lng: position.coords.longitude } : undefined;

  const otherIncidents = incidents.filter(other => other.id !== incident?.id);

  if (incident == null) {
    return <div className="px-6 py-2 text-sm font-semibold">Loading...</div>;
  }
    ...
    <div>
      ...
      <div className="px-6 py-2 text-xs">
        <IncidentDetailSection title={incident.type}>
          {incident.subType && <div className="font-bold text-blue-800">{incident.subType}</div>}
