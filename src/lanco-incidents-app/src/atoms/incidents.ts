...
const reducer = (state: IncidentsState, action: IncidentsAction): IncidentsState => {
  switch (action.type) {
    case "LOAD": {
...
function updateIncidents(originalList: IncidentRecord[], newList: IncidentRecord[]): IncidentRecord[] {
  ...
  const areNotIn = (a: IncidentRecord[]) => (b: IncidentRecord) => !a.some(isEqual(b));
...
const persistedIncidentsAtom = atomWithStorage<Array<IncidentRecord>>("incidents", []);
