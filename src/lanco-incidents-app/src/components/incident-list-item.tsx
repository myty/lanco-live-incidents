...
export const IncidentListItem: React.FC<IncidentListItemProps> = ({ incident }) => {
  return (
      ...
      <Link className="w-full" to={`/incidents/${incident.id}`}>
        <div className="flex items-center flex-1 p-4 bg-white rounded-md shadow cursor-pointer select-none">
          <div className="flex-grow pr-2">
            <div className="text-sm font-bold text-gray-900">{incident.type}</div>
          ...
          <div className="flex-shrink-0 w-16 ml-2">
            ...
            <div>{incident.distance == null ? "--" : `${incident.distance.toFixed(2)} mi`}</div>
